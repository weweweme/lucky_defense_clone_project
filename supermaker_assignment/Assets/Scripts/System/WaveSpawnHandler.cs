...
namespace System
{
    ...
    public static class WaveSpawnHandler
    {
        ...
        public static async UniTask HandleWaveSpawnAsync(uint currentWave, CancellationToken token)
        {
            if (currentWave % 10 == 0)
            {
        ...
        /// <param name="token">작업 취소 토큰</param>
        private static async UniTask SpawnNormalWaveAsync(uint waveNumber, CancellationToken token)
        {
        ...
        /// <param name="waveNumber">웨이브 번호</param>
        /// <param name="token">작업 취소 토큰</param>
        private static async UniTask SpawnBossWaveAsync(uint waveNumber, CancellationToken token)
        {
            // 여기에 보스 스폰 로직 추가 예정
            await UniTask.CompletedTask;
        }
